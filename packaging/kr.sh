#!/usr/bin/env bash
# kr — MLRift / KernRift fat binary launcher
#
# Termux on Android 14+ denies raw execve of files in /data/data/<app>/.
# The runner binary (kr-bin) does a raw syscall and gets EACCES when it
...
if [ "$status" -eq 120 ] && [ -x ./kr-exec ]; then
    shift  # drop the .mlrbo / .krbo path; remaining args go to the slice
    exec ./kr-exec "$@"
fi
