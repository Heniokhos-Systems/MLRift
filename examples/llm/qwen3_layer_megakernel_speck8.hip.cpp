// Slice 4.15 (PLD speck8) — qwen3-0.6B layer mega-kernel for M=8 queries.
//
// Variant of qwen3_layer_megakernel_speck4.hip.cpp (slice 4.14) that
// processes M_EFF=8 query tokens per dispatch.  Strategy unchanged: each
// matmul WG reads its weight row ONCE and computes M_EFF dot-products
// against M_EFF input vectors; per-stream phases (1, 5, 7, 11, 15) loop
// m=0..M_EFF-1 internally.
//
// Build:
//   hipcc --offload-arch=gfx1100 --genco -O3 \
//       examples/llm/qwen3_layer_megakernel_speck8.hip.cpp \
//       -o /tmp/qwen3_layer_megakernel_speck8.co
...
// ────────────────────────────────────────────────────────────────
__device__ __forceinline__
void phase1_input_rmsnorm_speck8(unsigned int wg_id, unsigned int lane,
                                  const float *in_residual_8,
                                  const float *in_norm_g,
                                  float *b_x_norm_g_8) {
    if (wg_id != 0) return;
    for (int m = 0; m < M_EFF; m++) {
        const float *in_m = in_residual_8 + m * HIDDEN;
        float *out_m = b_x_norm_g_8 + m * HIDDEN;
        float ssq = 0.0f;
...
// M_EFF dot products (one per query token).  Generalized over M.
// ────────────────────────────────────────────────────────────────
__device__ __forceinline__
void phase3_qkv_matmul_speck8(unsigned int wg_id, unsigned int lane,
                               const unsigned short *qkv_w,
                               const float *b_x_norm_g_8,
                               float *b_qkv_g_8) {
    ...
    for (unsigned int r_off = 0; r_off < ROWS_PER_WG; r_off++) {
        ...
        for (unsigned int kp = lane; kp < HIDDEN / 2; kp += WAVE) {
            ...
            #pragma unroll
            for (int m = 0; m < M_EFF; m++) {
                acc[m] += w0 * b_x_norm_g_8[m * HIDDEN + k]
                       +  w1 * b_x_norm_g_8[m * HIDDEN + k + 1];
            }
        }
        for (int offset = WAVE / 2; offset > 0; offset >>= 1) {
            #pragma unroll
...
// 256 WGs at M=8.  Looping m inside a WG (the previous design) caused a
// per-stream numerical drift for m>=1 — flattening to one WG per
// (m, head) matches the M=1 code path exactly.
//
// rope_cos/rope_sin layout: [M_EFF, HEAD_DIM/2].
// ────────────────────────────────────────────────────────────────
__device__ __forceinline__
void phase5_qkv_split_qknorm_rope_speck8(unsigned int wg_id, unsigned int lane,
                                          const float *q_norm_g, const float *k_norm_g,
                                          const float *rope_cos_8, const float *rope_sin_8,
                                          const float *b_qkv_g_8, float *b_attn_q_g_8,
                                          float *kc_layer, float *vc_layer,
                                          unsigned long long pos_base) {
    constexpr unsigned int N_Q_TASKS  = M_EFF * N_HEADS;     // 128
    constexpr unsigned int N_K_TASKS  = M_EFF * N_KV_HEADS;  // 64
    constexpr unsigned int N_V_TASKS  = M_EFF * N_KV_HEADS;  // 64
    constexpr unsigned int N_TASKS    = N_Q_TASKS + N_K_TASKS + N_V_TASKS;  // 256
    ...
    if (wg_id < N_Q_TASKS) {
        ...
        const float *src = b_qkv_g_8 + m * QKV_DIM + head * HEAD_DIM;
        float ssq = 0.0f;
        ...
        const float *rc = rope_cos_8 + m * half;
        const float *rs = rope_sin_8 + m * half;
        float *qdst = b_attn_q_g_8 + m * Q_DIM + head * HEAD_DIM;
        ...
        const float *src = b_qkv_g_8 + m * QKV_DIM + Q_DIM + head * HEAD_DIM;
        float ssq = 0.0f;
        for (unsigned int i = lane; i < HEAD_DIM; i += WAVE) {
            float v = src[i];
            ssq += v * v;
        ...
        unsigned int m    = t / N_KV_HEADS;
        unsigned int head = t % N_KV_HEADS;
        const float *src = b_qkv_g_8 + m * QKV_DIM + Q_DIM + KV_DIM + head * HEAD_DIM;
        unsigned long long pos_m = pos_base + (unsigned long long)m;
...
// m=0..M_EFF-1.  Causal: query m attends positions [0..pos_base+m].
// ────────────────────────────────────────────────────────────────
#define ATTN_COOP 4
__device__ __forceinline__
void phase7_attn_decode_speck8(unsigned int wg_id, unsigned int lane,
                                const float *b_attn_q_g_8,
                                const float *kc_layer, const float *vc_layer,
                                float *b_attn_out_g_8,
                                unsigned long long pos_base) {
    ...
    for (int m = 0; m < M_EFF; m++) {
        ...
        if (lane < OUT_PER_WG) {
            ...
            b_attn_out_g_8[m * Q_DIM + q_head * HEAD_DIM + i] = acc;
        }
...
// ────────────────────────────────────────────────────────────────
__device__ __forceinline__
void phase9_oproj_residual_speck8(unsigned int wg_id, unsigned int lane,
                                   const unsigned short *o_w,
                                   const float *b_attn_out_g_8,
                                   const float *in_residual_8,
                                   float *b_mid_g_8) {
    ...
    for (unsigned int r_off = 0; r_off < ROWS_PER_WG; r_off++) {
        ...
        for (unsigned int kp = lane; kp < Q_DIM / 2; kp += WAVE) {
            ...
            #pragma unroll
            for (int m = 0; m < M_EFF; m++) {
                acc[m] += w0 * b_attn_out_g_8[m * Q_DIM + k]
                       +  w1 * b_attn_out_g_8[m * Q_DIM + k + 1];
            }
        }
        for (int offset = WAVE / 2; offset > 0; offset >>= 1) {
            #pragma unroll
        ...
        if (lane == 0) {
            #pragma unroll
            for (int m = 0; m < M_EFF; m++) {
                b_mid_g_8[m * HIDDEN + row] = in_residual_8[m * HIDDEN + row] + acc[m];
            }
        }
    }
}
...
// ────────────────────────────────────────────────────────────────
__device__ __forceinline__
void phase11_post_norm_speck8(unsigned int wg_id, unsigned int lane,
                               const float *post_norm_g,
                               const float *b_mid_g_8, float *b_mid_norm_g_8) {
    if (wg_id != 0) return;
    for (int m = 0; m < M_EFF; m++) {
        const float *in_m = b_mid_g_8 + m * HIDDEN;
        float *out_m = b_mid_norm_g_8 + m * HIDDEN;
        float ssq = 0.0f;
        for (unsigned int i = lane; i < HIDDEN; i += WAVE) {
            float v = in_m[i];
            ssq += v * v;
...
// ────────────────────────────────────────────────────────────────
// Phase 15 — silu_mul: gu_scratch_8[m, 0..FF] = silu(gate) * up.
// ────────────────────────────────────────────────────────────────
__device__ __forceinline__
void phase15_silu_mul_speck8(unsigned int wg_id, unsigned int lane,
                              float *gu_scratch_8) {
    constexpr unsigned int N_OUT = 2 * FF;
    constexpr unsigned int ELEMS_PER_WG = (FF + WG_PERSIST * WAVE - 1)
                                          / (WG_PERSIST * WAVE);
    for (int m = 0; m < M_EFF; m++) {
        float *gu_m = gu_scratch_8 + m * N_OUT;
...
// ────────────────────────────────────────────────────────────────
__device__ __forceinline__
void phase17_down_residual_speck8(unsigned int wg_id, unsigned int lane,
                                   const unsigned short *down_w,
                                   const float *gu_scratch_8,
                                   const float *b_mid_g_8,
                                   float *out_residual_8) {
    constexpr unsigned int N_OUT = 2 * FF;
    constexpr unsigned int ROWS_PER_WG = (HIDDEN + WG_PERSIST - 1) / WG_PERSIST;
    for (unsigned int r_off = 0; r_off < ROWS_PER_WG; r_off++) {
        unsigned int row = wg_id + r_off * WG_PERSIST;
        ...
        for (unsigned int kp = lane; kp < FF / 2; kp += WAVE) {
            ...
            #pragma unroll
            for (int m = 0; m < M_EFF; m++) {
                acc[m] += w0 * gu_scratch_8[m * N_OUT + k]
                       +  w1 * gu_scratch_8[m * N_OUT + k + 1];
            }
        }
        for (int offset = WAVE / 2; offset > 0; offset >>= 1) {
            #pragma unroll
        ...
        if (lane == 0) {
            #pragma unroll
            for (int m = 0; m < M_EFF; m++) {
                out_residual_8[m * HIDDEN + row] = b_mid_g_8[m * HIDDEN + row] + acc[m];
            }
        }
    }
}
...
// ────────────────────────────────────────────────────────────────
// MEGA-KERNEL ENTRY (24 kernargs, same layout as M=4 variant).
// ────────────────────────────────────────────────────────────────
extern "C" __global__ __launch_bounds__(WAVE)
void qwen3_layer_megakernel_speck8(
    const float          *in_residual_8,         //  0 [M_EFF, HIDDEN]
    float                *out_residual_8,        //  1 [M_EFF, HIDDEN]
    ...
    const float          *rope_cos_8,            // 10 [M_EFF, HEAD_DIM/2]
    const float          *rope_sin_8,            // 11 [M_EFF, HEAD_DIM/2]
    float                *kc_layer,              // 12
    float                *vc_layer,              // 13
    unsigned long long    pos_base,              // 14
    unsigned int         *barrier_counter,       // 15
    float                *gu_scratch_8,          // 16 [M_EFF, 2*FF]
    float                *b_x_norm_g_8,          // 17 [M_EFF, HIDDEN]
    float                *b_qkv_g_8,             // 18 [M_EFF, QKV_DIM]
    float                *b_attn_q_g_8,          // 19 [M_EFF, Q_DIM]
    float                *b_mid_g_8,             // 20 [M_EFF, HIDDEN]
    float                *b_mid_norm_g_8,        // 21 [M_EFF, HIDDEN]
    float                *b_attn_out_g_8,        // 22 [M_EFF, Q_DIM]
    unsigned long long    max_phase              // 23
) {
    unsigned int wg_id = blockIdx.x;
    unsigned int lane  = threadIdx.x;

    phase1_input_rmsnorm_speck8(wg_id, lane, in_residual_8, in_norm_g, b_x_norm_g_8);
    mega_barrier(barrier_counter, 0);
    if (max_phase < 1) return;

    phase3_qkv_matmul_speck8(wg_id, lane, qkv_w, b_x_norm_g_8, b_qkv_g_8);
    mega_barrier(barrier_counter, 1);
    if (max_phase < 2) return;

    phase5_qkv_split_qknorm_rope_speck8(wg_id, lane, q_norm_g, k_norm_g,
                                         rope_cos_8, rope_sin_8,
                                         b_qkv_g_8, b_attn_q_g_8,
                                         kc_layer, vc_layer, pos_base);
    mega_barrier(barrier_counter, 2);
    if (max_phase < 3) return;

    phase7_attn_decode_speck8(wg_id, lane, b_attn_q_g_8, kc_layer, vc_layer,
                               b_attn_out_g_8, pos_base);
    mega_barrier(barrier_counter, 3);
    if (max_phase < 4) return;

    phase9_oproj_residual_speck8(wg_id, lane, o_w, b_attn_out_g_8,
                                  in_residual_8, b_mid_g_8);
    mega_barrier(barrier_counter, 4);
    if (max_phase < 5) return;

    phase11_post_norm_speck8(wg_id, lane, post_norm_g, b_mid_g_8, b_mid_norm_g_8);
    mega_barrier(barrier_counter, 5);
    if (max_phase < 6) return;

    phase13_gate_up_matmul_speck8(wg_id, lane, gate_up_w, b_mid_norm_g_8, gu_scratch_8);
    mega_barrier(barrier_counter, 6);
    if (max_phase < 7) return;

    phase15_silu_mul_speck8(wg_id, lane, gu_scratch_8);
    mega_barrier(barrier_counter, 7);

    phase17_down_residual_speck8(wg_id, lane, down_w, gu_scratch_8, b_mid_g_8, out_residual_8);
}
